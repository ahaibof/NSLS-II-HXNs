...
#            mov_to_image_cen_smar(-1)
            #RE(fly1d(zps.zpssx,-15,15,100,0.5))
            #mov_to_line_center(scan_id=-1,elem='Au_M',threshold=0.2,moveflag=1,movepiezoflag=0)
...
#            mov_to_image_cen_smar(-1)
            #RE(fly1d(zps.zpssz,-15,15,100,0.5))
            #mov_to_line_center(scan_id=-1,elem='Au_M',threshold=0.2,moveflag=1,movepiezoflag=0)
...
def load_merlin_frame(sid=-1):
    df = get_table(db[sid],fill=False)
    num_frame, count = np.shape(df)
    roi_data = np.zeros(num_frame)
    for i in range(1,num_frame+1):
        tmp = 1.* np.squeeze(np.asarray(filestore.api.get_data(df['merlin1'][i])))
        roi_data[i-1] = np.sum(tmp)
    roi_data = np.max(roi_data) - roi_data
