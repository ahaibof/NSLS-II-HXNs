...
CrXANES= {'high_e':6.03, 'low_e':5.97, 
          ...
          'high_e_crl':12, 'low_e_crl':12,'crl_comb':(8)
          'high_e_zpz1':50.92, 'zpz1_slope':-5.9,
          'energylist':[], 'mirror': 'Si', 'pitch' :0.4665,'m2p':1.3060 )
...
def zp_list_xanes2d(e_list,dets,mot1,x_s,x_e,x_num,mot2,y_s,y_e,y_num,accq_t,
                    xcen = 0, ycen = 0, alignElem = 'Fe', alignX = (-2,2,100,0.05),
                    alignY = (-2,2,100,0.05), pdfElem = ['Fe','Zn'],
    ...
    for i in range (len(e_list)):
        ...
        if e_list['energy'][i]<0: # for special scans if no align elem available
            ...
            xcen = return_line_center(-1,alignElem,0.7)
            yield from bps.mov(zpssx, xcen)
            yield from fly1d(dets,zpssy,alignY[0],alignY[1],alignY[2],alignY[3])
            ycen = return_line_center(-1,alignElem,0.7)
            yield from bps.mov(zpssy, ycen)
        ...
        e_list.to_csv(os.path.join(saveLogFolder, filename), float_format= '%.3f')
